...
export class EvaluationRubricService {
   ...
   */
  getRubricByPublicId(publicId: string): Observable<EvaluationRubricResponse> {
  ...
  updateRubric(
    publicId: string,
    rubric: EvaluationRubricUpdateRequest
  ): Observable<EvaluationRubricUpdateResponse> {
    // Uncomment the line below when the backend is ready
   ...
   */
  createRubric(
    rubric: EvaluationRubricRequest
  ): Observable<EvaluationRubricResponse> {
    // Uncomment the line below when the backend is ready
  ...
  changeRubricStatus(
    publicId: string,
    status: string
  ): Observable<EvaluationRubricStatusResponse> {
    // Uncomment the line below when the backend is ready
  ...
  searchRubrics(filters: { [key: string]: any }): Observable<EvaluationRubricResponse[]> {
    ...
    for (const key in filters) {
      if (filters[key]) {
        filteredRubrics = filteredRubrics.filter((r) => r[key] === filters[key]);
      }
    }
